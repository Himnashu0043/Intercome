...
import com.application.intercom.databinding.FragmentUnpaidExpensesManagerBinding
...
class UnpaidExpensesManagerFragment : Fragment(), UnpaidExpensesAdapter.Delete {
    lateinit var binding: FragmentUnpaidExpensesManagerBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentUnpaidExpensesManagerBinding.inflate(layoutInflater)
    ...
    private fun unpaidExpensesList() {
        val token = prefs.getString(
            SessionConstants.TOKEN, GPSService.mLastLocation?.latitude.toString()
        )
        viewModel.unpaidExpensesList(token, "PENDING", buildingId)
    ...
    private fun observer() {
                    ...
                    it.value.let {
                        if (it.status == AppConstants.STATUS_SUCCESS) {
                            ...
                            if (list.isEmpty()) {
                                ...
                                val adptr = UnpaidExpensesAdapter(requireContext(), list, this)
        ...
        viewModel.deleteUnPaidManagerLiveData.observe(this, androidx.lifecycle.Observer {
            when (it) {
                is EmpResource.Loading -> {
                    EmpCustomLoader.showLoader(requireActivity())
                }
    ...
    override fun unpaidDelete(position: Int, billId: String?) {
        val token = prefs.getString(
            SessionConstants.TOKEN, GPSService.mLastLocation?.latitude.toString()
        )
        viewModel.deleteUnPaidManager(token, billId ?: "")
    }
