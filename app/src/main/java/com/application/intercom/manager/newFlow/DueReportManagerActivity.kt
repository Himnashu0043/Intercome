...
import com.application.intercom.databinding.ActivityDueReportManagerBinding
...
import com.application.intercom.manager.newFlow.balancesheet.BalanceDueReportAdapter
...
class DueReportManagerActivity : AppCompatActivity() {
    lateinit var binding: ActivityDueReportManagerBinding
    private lateinit var viewModel: ManagerSideViewModel
    private var adptr: DueReportManagerAdapter? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityDueReportManagerBinding.inflate(layoutInflater)
    ...
    private fun dueReportManager() {
        ...
        val model = IncomeReportManagerPostModel("", prefs.getString(SessionConstants.NEWBUILDINGID, ""),"")
                ...
                is EmpResource.Success -> {
                    ...
                    it.value.let {
                        if (it.status == AppConstants.STATUS_SUCCESS) {
                            ...
                            adptr = DueReportManagerAdapter(this, list)
            ...
            BottomSheetDialog(this, R.style.CustomBottomSheetDialogTheme).apply {
               ...
               /* filterBottom.tvStartDate.setOnClickListener {
                    MaterialDatePicker.Builder.datePicker().setSelection(Date().time).build()
                        .apply {
                            show(
                                ...
                                this@DueReportManagerActivity.toString()
                            )
                ...
                filterBottom.commonBtn.tv.setOnClickListener {
                    ...
                    val model = IncomeReportManagerPostModel(endDate, prefs.getString(SessionConstants.NEWBUILDINGID, ""),startDate)
                    viewModel.dueReportManager(token, model)
                    ...
                    if (year.isNotEmpty()) {
                        if (months.isEmpty()) {
                            ...
                            if (resultt != null) {
                                ...
                                val model = IncomeReportManagerPostModel(setNewFormat(end) ?: "", prefs.getString(SessionConstants.NEWBUILDINGID, ""),setNewFormat(
                                    start
                                ) ?: "")
                                viewModel.dueReportManager(token, model)
                                dismiss()
