package com.application.intercom.owner.activity.ownerProperty
...
import com.application.intercom.databinding.ActivityOwnerProperty2Binding
...
import com.application.intercom.user.property.NewUserPropertyMapAdapter
import com.application.intercom.user.property.PropertyFilterByBottomSheet
import com.application.intercom.user.property.PropertyListingAdapter
...
class OwnerPropertyActivity : BaseActivity<ActivityOwnerProperty2Binding>(),
    ...
    NewUserPropertyMapAdapter.CLickRcyOnMap, ProfileAdapter.ProfileClick {
    /////side menu
    ...
    private var key: String = "ownerSide_property"
    ...
    private var getamem_List = ArrayList<PropertyLisRes.Data.Amentity>()
    ...
    private var getOwnerpropertyId: String = ""
    ...
    private var get_filter_lati: Double = 0.0
    private var get_filter_longi: Double = 0.0
    private var rent_sell: String? = null
    private var propertyType: String? = null
    private var from: String = ""
    override fun getLayout(): ActivityOwnerProperty2Binding {
        return ActivityOwnerProperty2Binding.inflate(layoutInflater)
    }
...
//        )
        ...
        println("-----from_Filter$from")
        if (from.equals("from_filter")) {
            ...
            get_filter_lati = intent.getDoubleExtra("getLat", 0.0)
            get_filter_longi = intent.getDoubleExtra("getLong", 0.0)
        }
        initView()
        listener()
        ///side menu
...
//        profile_list.add(ProfileModal(R.drawable.parking_icon, "My Parking"))
    ...
    private fun setPropertyAdapter(list: ArrayList<PropertyLisRes.Data> = ArrayList()) {
        ...
        mAdapter = PropertyListingAdapter(this, list, this)
    ...
    private fun addFav() {
        ...
        viewModel.userAddfavProperty(token, getOwnerpropertyId, null)
    ...
    private fun listener() {
        ...
        binding.imageView25.setOnClickListener {
            binding.ownerPropertyDrw.openDrawer(GravityCompat.START)
        }
        ///side mneu
        /*binding.imageView25.setOnClickListener {
            finish()
...
//                list_adapter?.notifiyData(serviceList)
        ...
        binding.edtSearch.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(p0: Editable?) {
                ...
                if (text.isNotEmpty()) {
                    if (text.length > 0) {
                        val tempFilterList = sendList.filter {
                        ...
                        binding.rvProperty.layoutManager =
                            LinearLayoutManager(this@OwnerPropertyActivity)
                        mAdapter = PropertyListingAdapter(
                            this@OwnerPropertyActivity,
                        ...
                        if (tempFilterList.isEmpty()) {
                            Toast.makeText(
                                this@OwnerPropertyActivity, "Data Not Found", Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
                } else {
                    binding.rvProperty.layoutManager =
                        LinearLayoutManager(this@OwnerPropertyActivity)
                    mAdapter = PropertyListingAdapter(
                        this@OwnerPropertyActivity, sendList, this@OwnerPropertyActivity
                    )
                    binding.rvProperty.adapter = mAdapter
                    mAdapter!!.notifyDataSetChanged()
        ...
        binding.ivMapview.setOnClickListener {
            if (btnMapList) {
                ...
                binding.ivMapview.setImageDrawable(
                    ContextCompat.getDrawable(
                        this, R.drawable.ic_list
                    )
                )
                binding.rcyMap.layoutManager =
                    LinearLayoutManager(this, RecyclerView.HORIZONTAL, false)
                mapRecyAdapter = NewUserPropertyMapAdapter(this, sendList, this)
    ...
    override fun onCLickProperty(
        ...
        startActivity(
            Intent(
                this, CompleteParkingToLetDetailsActivity::class.java
    ...
    override fun onClickMapRcy(
        ...
        startActivity(
            Intent(
                ...
                .putExtra("bath", bathRoom).putExtra("url", photoList).putExtra("dist", disT2)
                .putExtra("discription", discrption).putExtra("filedDetails_ID", flatDetailsId)
                .putExtra("building_Id", buildingId).putExtra("amm_list", amList)
                .putExtra("lati", lati).putExtra("longi", longi).putExtra("sendList", sendList)
                .putExtra("added", addedBy)
        )
    }
...
//                )
    ...
    fun closeDrawer() {
        if (binding.ownerPropertyDrw.isDrawerOpen(GravityCompat.START)) {
            binding.ownerPropertyDrw.closeDrawer(GravityCompat.START)
        }
    }
